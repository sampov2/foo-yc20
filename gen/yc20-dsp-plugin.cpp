  ...
  public:
	...
	virtual void compute (int fullcount, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (index = 0; index <= fullcount - 32; index += 32) {
			...
			// exec code
			for (int i=0; i<count; i++) {
				fRec119[i] = (fConst4 * (fRec119[i-1] + (fRec120[i] - fRec120[i-1])));
			}
			// post processing
			...
			// exec code
			for (int i=0; i<count; i++) {
				fZec223[i] = ((0.001f + (0.05f * fRec0[i])) * ((fSlow121 * ((fSlow120 * fZec222[i]) + ((fSlow118 * (fZec166[i] + (0.002f * (fRec159[i] * fSlow82)))) + ((fSlow116 * fZec212[i]) + ((fSlow114 * (fZec180[i] + (0.002f * fZec221[i]))) + ((fSlow112 * fZec213[i]) + ((fSlow110 * fZec215[i]) + (fSlow108 * fZec214[i])))))))) + ((fSlow106 * ((fSlow104 * (((fRec285[i] + fRec287[i]) + fRec289[i]) + fRec291[i])) + (fSlow103 * (((fRec293[i] + fRec295[i]) + fRec297[i]) + fRec299[i])))) + ((fRec301[i] * (0.5f + (4.5f * fRec302[i]))) + (fSlow101 * (max(fSlow100, (0.25f * (fYec100[i-4] + (fYec100[i-3] + (fYec100[i] + fYec100[i-2]))))) * ((0.5f * fZec215[i]) + (0.25f * ((fZec222[i] + fZec179[i]) + (0.008f * fZec221[i]))))))))));
			}
